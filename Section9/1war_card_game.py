class WarCardGame:
    ...
    def start_battle(self, card_from_war=None):
        ...
        computer_card = self._computer.drwa_card()
        print(f"Your Card :")
        player_card.show()
        print(f"\nComputer Card :")
        computer_card.show()
    ...
    def get_round_winner(self, player_card, computer_card):
        ...
        elif player_card.value < computer_card:
    ...
    def start_war(self, cards_from_battle):
        ...
        for i in range(3):
            player_card = self._player.draw_card()
            player_card.append(player_card)

            computer_card = self._computer.drwa_card()
            computer_card.append(computer_card)
        print("six hidden cards xxx xxx")
        self.start_battle(player_card + computer_card + cards_from_battle)
    ...
    def check_game_over(self):
        if self._player.has_empty_deck():
            ...
            print("|         GAME OVER        |")
            print("============================")
            print(f"Excellent.  You win,{self._player.name}! Congratulation")
            return True
        else:
            return False
